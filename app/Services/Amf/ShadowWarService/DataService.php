...
class DataService
{
    ...
    /**
     * getSeason
     * Params: [charId, sessionKey, seasonNum (optional)]
     */
    public function getSeason($params)
    {
        ...
        $seasonNum = $params[2] ?? null; // PERUBAHAN: Tambah parameter season yang diminta
        
        Log::info("AMF ShadowWar.getSeason: Char $charId SeasonNum $seasonNum");

        // PERUBAHAN: Jika ada request season tertentu, ambil season itu
        if ($seasonNum !== null) {
            $activeSeason = ShadowWarSeason::where('num', $seasonNum)->first();
        } else {
            // Jika tidak, ambil season aktif
    ...
    public function getStatus($params)
    {
        ...
        // PERUBAHAN: Ubah validasi agar RANK_GENIN level 1 bisa akses
        if ($char->rank < Character::RANK_GENIN || $char->level < 1) {
            return [
                'status' => 2,
                'result' => 'Your character must be at least GENIN rank and level 1'
            ];
        }
        ...
        if (!$activeSeason) {
            return [
                'status' => 2,
                'result' => 'Coming soon when its hosted!'
            ];
        }
