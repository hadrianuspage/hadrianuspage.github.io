...
class BattleService
{
    ...
    public function refillEnergy($params)
    {
        ...
        if (!$activeSeason) {
            return ['status' => 2, 'result' => 'Coming soon when its hosted!'];
        }
    ...
    // PERUBAHAN: Ubah buildEnemies untuk menyesuaikan level musuh dengan level player
    private function buildEnemies(?int $charId): array
    {
        ...
        // Cari musuh dengan level yang sama atau mendekati
        $query = Character::query()
            ->whereBetween('level', [max(1, $playerLevel - 2), $playerLevel + 5])
            ->where('id', '!=', $charId);
        
        $characters = $query->inRandomOrder()->limit(5)->get();
        
        // Jika tidak ada yang level sama/mendekati, ambil yang level lebih tinggi
        if ($characters->isEmpty()) {
            $characters = Character::query()
                ->where('level', '>', $playerLevel)
                ->where('id', '!=', $charId)
                ->orderBy('level', 'asc')
                ->limit(5)
                ->get();
        ...
        // Fallback jika masih kosong, ambil level lebih rendah
        if ($characters->isEmpty()) {
            $characters = Character::query()
                ->where('level', '<', $playerLevel)
                ->where('id', '!=', $charId)
        ...
        // Fallback terakhir: ambil karakter sendiri
