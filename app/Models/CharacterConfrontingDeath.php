...
class CharacterConfrontingDeath
{
    ...
    public static function getData($characterId)
    {
        ...
        if (!Session::has($sessionKey)) {
            $defaultData = [
                ...
                'milestone_data' => [
                    ['claimed' => false], // 10 battles
                    ['claimed' => false], // 50 battles
                    ['claimed' => false], // 100 battles
                    ['claimed' => false], // 200 battles
                    ['claimed' => false], // 300 battles
                    ['claimed' => false], // 400 battles
                    ['claimed' => false], // 600 battles
                    ['claimed' => false]  // 750 battles
                ]
            ];
            
            Session::put($sessionKey, $defaultData);
            Log::info("ConfrontingDeath: Created new session data for char $characterId");
        }
    ...
    public static function setData($characterId, $data)
    {
        ...
        Log::info("ConfrontingDeath: Updated session data for char $characterId - Energy: {$data['energy']}, Battles: {$data['total_battles']}");
    }
    ...
    public static function autoRefillEnergy($characterId)
    {
        ...
        if ($energyToAdd > 0) {
            ...
            Log::info("ConfrontingDeath Auto refilled energy: {$oldEnergy} -> {$data['energy']} (+{$energyToAdd})");
        }
     ...
     */
    public static function incrementBattles($characterId)
    {
        $data = self::getData($characterId);
        $oldBattles = $data['total_battles'];
        $data['total_battles']++;
        self::setData($characterId, $data);
        
        Log::info("ConfrontingDeath Battles incremented: {$oldBattles} -> {$data['total_battles']}");
        return $data;
    }
    ...
    public static function useEnergy($characterId, $amount = 1)
    {
        ...
        if ($data['energy'] >= $amount) {
            ...
            Log::info("ConfrontingDeath Energy used: -{$amount}, {$oldEnergy} -> {$data['energy']}");
            return true;
        }
        
        Log::warning("ConfrontingDeath Energy insufficient: {$data['energy']} < $amount");
        return false;
    }
    ...
    public static function refillEnergyWithToken($characterId)
    {
        ...
        Log::info("ConfrontingDeath Energy refilled with token: {$oldEnergy} -> " . self::MAX_ENERGY);
        return $data;
    }
    ...
    public static function clearData($characterId)
    {
        ...
        Log::info("ConfrontingDeath: Cleared session data for char $characterId");
    }
}
