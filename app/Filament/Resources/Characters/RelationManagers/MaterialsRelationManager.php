...
class MaterialsRelationManager extends RelationManager
{
    ...
    public function form(Schema $schema): Schema
    {
        return $schema
            ->components([
                Forms\Components\Hidden::make('category')->default('material'),
                Forms\Components\Select::make('item_id')
                    ->label('Material')
                    ->options(\App\Models\Item::where('item_id', 'like', 'material\_%')->pluck('name', 'item_id'))
                    ->required()
                    ->searchable(),
    ...
    {
        return $table
            ->modifyQueryUsing(fn (Builder $query) => $query->where('item_id', 'like', 'material\_%'))
            ->columns([
