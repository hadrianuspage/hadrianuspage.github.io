...
class EssentialsRelationManager extends RelationManager
{
    protected static string $relationship = 'items';
    protected static ?string $recordTitleAttribute = 'item_id';
    protected static ?string $title = 'Essentials';
    ...
    public function form(Schema $schema): Schema
    {
        return $schema
            ->components([
                Forms\Components\Hidden::make('category')->default('essential'),
                Forms\Components\Select::make('item_id')
                    ->label('Essential Item')
                    ->options(\App\Models\Item::where('item_id', 'like', 'essential\_%')->pluck('name', 'item_id'))
                    ->required()
                    ->searchable(),
    ...
    {
        return $table
            ->modifyQueryUsing(fn (Builder $query) => $query->where('item_id', 'like', 'essential\_%'))
            ->columns([
